...
class AddingLRU(nn.Module):
    ...
    def forward(self, x):
        # x: (B, T, 2)
        out = self.lru(x)          # (B, T, H)
        last = out[:, -1, :]       # last timestep
        return self.head(last)     # (B, 1)

def train_lru(max_steps=300_000, batch_size=20, seq_len=100, hidden_size=128):
    model = AddingLRU(hidden_size=hidden_size)
    model.train()
    ...
    for step in range(1, max_steps + 1):
        ...
        x = torch.tensor(seqs, dtype=torch.float32)      # (B,T,2)

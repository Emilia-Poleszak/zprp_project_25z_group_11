...
class Components:
    ...
    def generate_adding(self) -> tuple[list[tuple[float, float]], float]:
        ...
        """
        first = [self.rng.uniform(self.low, self.high)
        ...
        possible_indices = [i for i in range(
            self.length) if i != marked_one_idx]
        marked_two_idx = self.rng.choice(
            possible_indices[:self.length // 2 - 1])
    ...
    def generate_multiplication(self) -> tuple[list[tuple[float, float]], float]:
        ...
        marked_one_idx = self.rng.randint(0, min(9, self.length - 1))
        possible_indices = [i for i in range(
            self.length // 2 - 1) if i != marked_one_idx]
        marked_two_idx = self.rng.choice(
            possible_indices) if possible_indices else marked_one_idx
...
if __name__ == '__main__':
    ...
    if args.task == "adding":
        gen = Components(length=ADDING_SEQUENCE_LENGTH,
                         value_range=ADDING_RANGE, rng=args.rng)
        gen.save(args.num_sequences, ADDING_DATA_FILENAME)

    elif args.task == "multiplication":
        gen = Components(length=MULTIPLICATION_SEQUENCE_LENGTH,
                         value_range=MULTIPLICATION_RANGE, rng=args.rng)
